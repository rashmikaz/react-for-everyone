...
When rendering lists in React using .map(), each element needs a key.

- Why? Keys help React know which items in a list have changed, been added, or removed.
- What happens without keys? React will still render, but it shows a warning in the console and may re-render items inefficiently.
- What makes a good key? - A unique value for each item (like an id from your data),If no unique value exists, you can use the array index, but this is not recommended for dynamic lists.
